...
{\LARGE \bfseries Servicio de locación para evaluar la influencia del cambio climático en los eventos de El Niño y sus impactos en el Perú \par}
% \vspace{0.2cm}
...
{\normalsize Repositorio: \archivo{https://github.com/Japq91/e1_smn} \par}
...
% la misma redacción completa ("48 modelos... 142 de 142 combinaciones...").
...
El resultado operativo de este entregable es un \emph{pipeline} reproducible (repositorio \archivo{e1_smn}, orquestado por un \'unico punto de entrada \texttt{run.sh}) que descarga, homogeneiza, enmascara y valida la variable \texttt{tos} (temperatura superficial del mar, tabla CMOR \texttt{Omon}) de todo modelo CMIP6 disponible con los experimentos \texttt{historical}, \texttt{ssp245} y \texttt{ssp585}, junto con la referencia observacional ERSSTv5 de NOAA. Del universo de 102 modelos candidatos (46 de un barrido inicial del vocabulario CMIP6 completo m\'as 56 citados en la literatura del proyecto), 47 modelos quedaron completos en los tres experimentos y aprobaron el control de calidad final (141 de 141 combinaciones modelo-experimento); un modelo adicional, MIROC-ES2H, qued\'o con descarga parcial (\'unicamente \texttt{historical}) y se excluye de este conteo aunque figure en el inventario t\'ecnico (Secci\'on~\ref{sec:resultados}). Entre los 47 completos se resolvi\'o, entre otros, un bug de contaminaci\'on num\'erica de FGOALS-f3-L mediante un filtro de seguridad por valor f\'isico (Secci\'on~\ref{sec:paso05}).

En paralelo a la construcci\'on del dato, este informe desarrolla en profundidad el marco te\'orico exigido por los t\'erminos de referencia (TdR): la partici\'on de incertidumbre de \citet{hawkins2012}, su adaptaci\'on por \citet{szabo2016}, el antecedente directo de \citet{bruno2023} sobre CMIP6/Sudam\'erica, el precedente institucional de \citet{alvarez2024}, la caracterizaci\'on del Ni\~no costero de \citet{takahashi2019}, y un estado del arte de otras metodolog\'ias del TOE del cual se deriva la justificaci\'on expl\'icita de los dos m\'etodos de TOE que se aplicar\'an en el Entregable~4: M1 (\citealp{szabo2016}) y \emph{Time of Detection} (ToD, \citealp{gopika2024}).

% ============================================================
\section{Alcance --- informe 1}
\label{sec:alcance}
% ============================================================

El TdR final del servicio (\archivo{TDR_COSTA_NORTE_p1_completado_4meses}, en adelante ``el TdR''; sin anexos t\'ecnicos adjuntos) define el Entregable~1 --actividad~(a), plazo m\'aximo 30 d\'ias-- como: \emph{``Realizar la revisi\'on de metodolog\'ias sobre tiempo de emergencia (TOE), recopilaci\'on de datos de la informaci\'on de temperatura superficial del mar (TSM), y simulaciones hist\'oricas y futuras de los modelos clim\'aticos globales CMIP6 para los escenarios SSP2-4.5 y SSP5-8.5, correspondientes a las regiones Ni\~no~3.4 y Ni\~no~1+2''}. Para operacionalizar este mandato, el equipo t\'ecnico lo descompone en cinco tareas concretas: (i) revisar antecedentes cient\'ificos (ENOS, Ni\~no costero, partici\'on de incertidumbre de Hawkins y Sutton, la aplicaci\'on de Szab\'o, la tesis de Bruno Ram\'irez, el estudio previo de \'Alvarez S\'anchez, y otras metodolog\'ias del TOE); (ii) consolidar el conjunto de modelos CMIP6 disponibles con la variable requerida; (iii) recopilar la referencia observacional ERSSTv5; (iv) definir los dominios oceanogr\'aficos y los periodos temporales de an\'alisis; y (v) homogeneizar unidades, calendarios, resoluciones espaciales y m\'ascaras oc\'eano-tierra entre todas las fuentes. Las tareas (i) son de revisi\'on bibliogr\'afica y se resuelven en la Secci\'on~\ref{sec:marco} de este informe; las tareas (ii)--(v) son computacionales y se resuelven mediante el \emph{pipeline} descrito en las Secciones~\ref{sec:arquitectura} y~\ref{sec:scripts}.

Es importante precisar el alcance negativo de este entregable: no corresponde a\'un evaluar el desempe\~no hist\'orico de los modelos (climatolog\'ias, sesgos, m\'etricas estad\'isticas e \'indices, con fines de selecci\'on de modelos --Entregable~2--), no corresponde proyectar el estado medio ni la variabilidad del ENOS ni evaluar \'indices como ONI e ICEN sobre proyecciones (Entregable~3), y no corresponde estimar las fuentes de incertidumbre, calcular la raz\'on se\~nal-ruido ni el TOE (Entregable~4). El TdR no exige gr\'aficas formales en el Entregable~1; las figuras de la Secci\'on~\ref{sec:resultados} son verificaciones exploratorias de que el dato qued\'o correctamente homogeneizado, no resultados cient\'ificos del proyecto.
% REVISAR (redundancia, perfumería leve): esta misma advertencia ("figura
% exploratoria, no resultado formal") se repite, con distinta redacción pero
% el mismo contenido, en el pie de fig:mapas2d (Sección~\ref{sec:resultados}),
% en el pie de fig:boxplot, y de nuevo como primera frase de la Sección
% "Entregable 2: próximos pasos". Cuatro repeticiones del mismo descargo. Como
% ya queda establecido aquí (fuente única de verdad), en los tres lugares
% posteriores bastaría una cláusula corta ("ver alcance, Sección~\ref{sec:alcance}")
% en vez de reformular la idea completa cada vez.
...
\textbf{Ni\~no costero.} \citet{takahashi2019} caracterizan din\'amicamente el evento de El Ni\~no costero de 1925, distingui\'endolo de los eventos ``can\'onicos'' de gran escala (1982--1983, 1997--1998): el Ni\~no costero puede desarrollarse con condiciones c\'alidas en el Pac\'ifico oriental lejano y fr\'ias en el Pac\'ifico central, mediado por realimentaciones oc\'eano-atm\'osfera locales (viento-evaporaci\'on-TSM) y no necesariamente por ondas Kelvin ecuatoriales forzadas remotamente. Esta distinci\'on justifica emp\'iricamente por qu\'e el TdR exige tratar Ni\~no~1+2 y Ni\~no~3.4 como cajas independientes, no intercambiables.

Otras metodolog\'ias del TOE. El TdR pide, en su directiva general para el Entregable~1, la ``revisi\'on de metodolog\'ias sobre tiempo de emergencia (TOE)''; el equipo t\'ecnico interpreta este mandato de forma amplia y revisa tambi\'en metodolog\'ias de TOE m\'as all\'a de Hawkins-Sutton/Szab\'o. La Secci\'on~\ref{sec:referencias} sintetiza la \emph{bibliograf\'ia} de referencia --es decir, el conjunto acotado de art\'iculos sobre TOE identificado y revisado sistem\'aticamente para este proyecto, no un conjunto de datos-- usada para cumplir este punto, de la cual se deriva adem\'as el m\'etodo complementario ToD (Secci\'on~\ref{sec:toe_metodos}).

\subsection{Literatura sobre \emph{Time of Emergence}}
\label{sec:referencias}

El Cuadro~\ref{tab:referencias} sintetiza la bibliograf\'ia central sobre TOE revisada para este proyecto, incluyendo tanto los antecedentes exigidos expl\'icitamente por el TdR (Secci\'on~\ref{sec:antecedentes}) como literatura de refuerzo incorporada para robustecer la discusi\'on esperada en el Entregable~4.
...
El Cuadro~\ref{tab:modelos} lista los 48 modelos CMIP6 del inventario t\'ecnico para \texttt{tos}/\texttt{Omon} -- 47 con los tres experimentos completos (\texttt{historical}, \texttt{ssp245}, \texttt{ssp585}) y 1 (MIROC-ES2H, marcado en la tabla) con descarga parcial, \'unicamente \texttt{historical} -- junto con la instituci\'on que los genera (identificador \archivo{institution_id} de CMIP6), el pa\'is o consorcio de esa instituci\'on, y la resoluci\'on nominal original de la variante de grilla efectivamente descargada (antes del regrillado com\'un del Paso~04, Secci\'on~\ref{sec:scripts}) -- la m\'as gruesa disponible entre las variantes que publican los tres experimentos (Secci\'on~\ref{sec:desviacion}).

\begin{longtable}{@{}L{0.9cm}L{3.2cm}L{3.0cm}L{\dimexpr\textwidth-0.9cm-3.2cm-3.0cm-2.3cm-8\tabcolsep\relax}L{2.3cm}@{}}
\caption[Modelos CMIP6 descargados, con instituci\'on, pa\'is y resoluci\'on nominal.]{Modelos CMIP6 descargados: instituci\'on, pa\'is/consorcio y resoluci\'on nominal original
% (antes del regrillado com\'un a la grilla de ERSSTv5)
.}
\label{tab:modelos} \\
\toprule
\textbf{ID} & \textbf{Modelo} & \textbf{Instituci\'on} & \textbf{Pa\'is / consorcio} & \textbf{Resoluci\'on} \\
\midrule
\endfirsthead
\toprule
\textbf{ID} & \textbf{Modelo} & \textbf{Instituci\'on} & \textbf{Pa\'is / consorcio} & \textbf{Resoluci\'on} \\
...
\bottomrule
\endfoot
M1  & ACCESS-CM2 & CSIRO-ARCCSS & Australia & 250 km \\
M2  & ACCESS-ESM1-5 & CSIRO & Australia & 250 km \\
M3  & AWI-CM-1-1-MR & AWI & Alemania & 25 km \\
M4  & BCC-CSM2-MR & BCC & China & 100 km \\
M5  & CAMS-CSM1-0 & CAMS & China & 100 km \\
M6  & CAS-ESM2-0 & CAS & China & 100 km \\
M7  & CESM2 & NCAR & Estados Unidos & 100 km \\
M8  & CESM2-WACCM & NCAR & Estados Unidos & 100 km \\
M9  & CIESM & THU & China & 100 km \\
M10 & CMCC-CM2-SR5 & CMCC & Italia & 100 km \\
M11 & CMCC-ESM2 & CMCC & Italia & 100 km \\
M12 & CNRM-CM6-1 & CNRM-CERFACS & Francia & 100 km \\
M13 & CNRM-CM6-1-HR & CNRM-CERFACS & Francia & 25 km \\
M14 & CNRM-ESM2-1 & CNRM-CERFACS & Francia & 100 km \\
M15 & CanESM5 & CCCma & Canad\'a & 100 km \\
M16 & CanESM5-1 & CCCma & Canad\'a & 100 km \\
M17 & CanESM5-CanOE & CCCma & Canad\'a & 100 km \\
M18 & EC-Earth3 & EC-Earth-Consortium & Consorcio europeo (EC-Earth) & 100 km \\
M19 & EC-Earth3-CC & EC-Earth-Consortium & Consorcio europeo (EC-Earth) & 100 km \\
M20 & EC-Earth3-Veg & EC-Earth-Consortium & Consorcio europeo (EC-Earth) & 100 km \\
M21 & EC-Earth3-Veg-LR & EC-Earth-Consortium & Consorcio europeo (EC-Earth) & 100 km \\
M22 & FGOALS-f3-L & CAS & China & 100 km \\
M23 & FGOALS-g3 & CAS & China & 100 km \\
M24 & FIO-ESM-2-0 & FIO-QLNM & China & 100 km \\
M25 & GFDL-CM4 & NOAA-GFDL & Estados Unidos & 25 km \\
M26 & GFDL-ESM4 & NOAA-GFDL & Estados Unidos & 50 km \\
M27 & GISS-E2-1-G & NASA-GISS & Estados Unidos & 250 km \\
M28 & GISS-E2-1-H & NASA-GISS & Estados Unidos & 250 km \\
M29 & GISS-E2-2-G & NASA-GISS & Estados Unidos & 250 km \\
M30 & HadGEM3-GC31-LL & MOHC & Reino Unido & 100 km \\
M31 & IITM-ESM & CCCR-IITM & India & 250 km \\
M32 & INM-CM4-8 & INM & Rusia & 100 km \\
M33 & INM-CM5-0 & INM & Rusia & 100 km \\
M34 & IPSL-CM6A-LR & IPSL & Francia & 100 km \\
M35 & KACE-1-0-G & NIMS-KMA & Corea del Sur & 250 km \\
M36 & KIOST-ESM & KIOST & Corea del Sur & 250 km \\
M37 & MCM-UA-1-0 & UA & Estados Unidos & 250 km \\
M38 & MIROC-ES2H$^{*}$ & MIROC & Jap\'on & 100 km \\
M39 & MIROC-ES2L & MIROC & Jap\'on & 100 km \\
M40 & MIROC6 & MIROC & Jap\'on & 100 km \\
M41 & MPI-ESM1-2-HR & DKRZ & Alemania & 50 km \\
M42 & MPI-ESM1-2-LR & DKRZ & Alemania & 250 km \\
M43 & MRI-ESM2-0 & MRI & Jap\'on & 100 km \\
M44 & NESM3 & NUIST & China & 100 km \\
M45 & NorESM2-LM & NCC & Noruega & 100 km \\
M46 & NorESM2-MM & NCC & Noruega & 100 km \\
M47 & TaiESM1 & AS-RCEC & Taiw\'an & 100 km \\
M48 & UKESM1-0-LL & MOHC & Reino Unido & 100 km \\
\end{longtable}
\noindent $^{*}$MIROC-ES2H tiene descarga parcial (\'unicamente \texttt{historical}; \texttt{ssp245} y \texttt{ssp585} quedaron incompletas, ver \archivo{informe/descarga_story.md} y la Figura~\ref{fig:qc}) y se excluye del conteo de 47 modelos completos aunque figure en el inventario t\'ecnico de 48 (Secci\'on~\ref{sec:resultados}).

La resoluci\'on nominal es una categor\'ia controlada del vocabulario CMIP6 (no una medida continua), consultada directamente contra el \'indice de b\'usqueda de ESGF para la variante de grilla efectivamente usada por cada modelo (campo \archivo{nominal_resolution} del \emph{dataset}); coincide, para los 45 modelos provenientes del barrido autom\'atico de \archivo{00b_build_model_list.py}, con el valor ya registrado en \archivo{config/models_seed_cmip6.csv}, y se verific\'o de la misma forma para los 3 modelos adicionales incorporados directamente desde la literatura del proyecto (BCC-CSM2-MR, CMCC-CM2-SR5, FGOALS-f3-L).
Esta resoluci\'on no es la que finalmente comparten los archivos procesados: el Paso~04 regrilla a todos por igual a la resoluci\'on de ERSSTv5 ($\sim$2\textdegree, Secci\'on~\ref{sec:desviacion}).

\subsection{Selecci\'on inicial de modelos}
\label{sec:desviacion}

El dise\~no original de este entregable tom\'o como punto de partida el conjunto de modelos previamente seleccionados por su desempe\~no en Sudam\'erica en \citet{bruno2023} (54 modelos validados en su trabajo de suficiencia profesional). Esa lista no se conserv\'o como archivo aparte en el repositorio; en su lugar, sus modelos quedaron trazables dentro de los art\'ifactos que s\'i persisten: de los 52 \archivo{source_id} distintos citados en \citet{bruno2023}, 34 coinciden directamente con el barrido autom\'atico completo de CMIP6 (\archivo{config/models_seed_cmip6.csv}), 16 m\'as quedaron registrados como candidatos de fuente alternativa en \archivo{config/models_missing_from_esgf.csv} (Secci\'on~\ref{sec:scripts}, Paso~00c), y 37 de los 52 terminaron formando parte del inventario final de modelos de este Entregable~1. La implementaci\'on de este Entregable~1 parti\'o conceptualmente de esa lista pero la ampli\'o deliberadamente: en lugar de restringir la descarga a esos modelos, el script \archivo{00b_build_model_list.py} (Secci\'on~\ref{sec:scripts}) enumera todos los \archivo{source_id} de CMIP6 publicados en ESGF con \texttt{tos}/\texttt{Omon}, y selecciona los que tienen los tres experimentos requeridos. La raz\'on de esta ampliaci\'on es doble: primero, la selecci\'on de \citet{bruno2023} se valid\'o sobre variables terrestres (temperatura del aire, precipitaci\'on) y sistemas sin\'opticos continentales, no sobre TSM oce\'anica en Ni\~no~3.4/1+2, por lo que no hab\'ia garant\'ia de que fuera exhaustiva para este dominio; segundo, ampliar el universo de candidatos a 102 modelos (46 del barrido inicial m\'as 56 citados en la literatura del proyecto, verificados con \archivo{00c_check_paper_models.py}) permite, en el Entregable~2, decidir con datos propios cu\'ales de esos 47 modelos completos son adem\'as representativos del ENOS observado --sin haber descartado de antemano candidatos v\'alidos por seguir estrictamente una lista pensada para otra variable.

De cada modelo se descarga, adem\'as, la variante de grilla (\archivo{grid_label}) m\'as gruesa disponible entre las que publican los tres experimentos (script \texttt{00b}), en lugar de la de mayor resoluci\'on: dado que el paso~04 regrilla igualmente todo a la resoluci\'on del dato observado de referencia (ERSSTv5, $\sim$2\textdegree), descargar variantes de alta resoluci\'on no aportar\'ia precisi\'on adicional al resultado final y s\'olo incrementar\'ia innecesariamente el volumen de datos y el tiempo de descarga.
...
\caption[Ubicaci\'on de las cajas oceanogr\'aficas Ni\~no~3.4 y Ni\~no~1+2.]{Regiones oceanogr\'aficas Ni\~no~3.4 (170\textdegree{}O--120\textdegree{}O, 5\textdegree{}S--5\textdegree{}N) y Ni\~no~1+2 (90\textdegree{}O--80\textdegree{}O, 10\textdegree{}S--0\textdegree{}) sobre el Pac\'ifico tropical oriental. Ni\~no~1+2, adyacente a la costa peruano-chilena y a la zona de surgencia de la Corriente de Humboldt, es la caja de mayor relevancia directa para el monitoreo del Ni\~no costero en Per\'u (\citealp{takahashi2019}), mientras que Ni\~no~3.4 es la caja est\'andar internacional del \'Indice Oce\'anico de El Ni\~no (ONI).}
\label{fig:region_nino}
\end{figure}

Definidos en \texttt{config/periods.yaml}: periodo de referencia/validaci\'on 1981--2014; proyecciones a 2036--2065 (P1) y 2071--2100 (P2); escenarios \texttt{ssp245} y \texttt{ssp585}. El rango temporal completo de descarga es 1850--2100 (\texttt{historical}: 1850--2014; \texttt{ssp245}/\texttt{ssp585}: 2015--2100). El criterio de selecci\'on de modelos vigente es \'unicamente la disponibilidad de \texttt{tos}/\texttt{Omon} en los tres experimentos \texttt{historical}, \texttt{ssp245} y \texttt{ssp585}.
...
Este m\'odulo vive en el repositorio \archivo{e1_smn}, con un \'unico punto de entrada (\texttt{run.sh}) y salidas versionadas en \texttt{data/processed/}, consumibles de forma independiente por el Entregable~2 sin necesidad de conocer el detalle interno de este m\'odulo.

\begin{lstlisting}[style=bashstyle,caption={Estructura interna del repositorio \texttt{e1\_smn}.},label={lst:tree}]
e1_smn/
...
|   `-- models_copernicus_ssp245_whitelist.csv
|-- scripts/
|   |-- 00_setup_env.sh
|   |-- 00b_build_model_list.py
|   |-- 00c_check_paper_models.py         # diagnostico, no forma parte de run.sh
|   |-- 01_query_esgf_catalog.py
...
\centering
\begin{tikzpicture}[
  box/.style={draw, rounded corners, fill=cyan!18, align=center, text width=3.4cm, minimum height=1.1cm, font=\small},
  boxaux/.style={draw, rounded corners, dashed, fill=green!20!cyan!15, align=center, text width=2.4cm, minimum height=1.1cm, font=\small},
...
\node[box] (S00b) at (0,-2.0) {00b\\Modelos candidatos};
\node[boxaux] (S00c) at (4.0,-2.0) {00c\\Verif. literatura};
\node[box] (S01) at (0,-4.1) {01\\Cat\'alogo ESGF};

\node[box] (S02) at (-2.6,-5.9) {02\\Descarga CMIP6};
\node[box] (S03) at (3.9,-5.9) {03\\Descarga ERSSTv5};
\node[box] (S02b) at (-2.6,-7.8) {02b\\Nodos ESGF alt.};
\node[box] (S02c) at (-2.6,-9.7) {02c\\Copernicus CDS};

\node[box] (S04) at (0.5,-11.7) {04\\Grilla com\'un};
\node[box] (S05) at (0.5,-13.6) {05\\M\'ascara oc\'eano-tierra};
\node[boxaux] (NB) at (4.4,-13.6) {Notebook\\gr\'aficos\\explorat.};
\node[box] (S06) at (0.5,-15.5) {06\\Control de calidad};
\node[box] (S07) at (0.5,-17.4) {07\\Inventario final};
...
% \draw[edg] (S02.west) -- ++(1.5,0) |- (S04.east);
% \draw[edg] (S02) to[out=210,in=180] (S04);
% \draw[edg] (S02) -- (S04);
\draw[edg] (S03) -- (S04);
\draw[edg] (S04) -- (S05);
\draw[edg] (S05) -- (S06);
...
Esta secci\'on describe la l\'ogica de cada script por su nombre y ruta, sin reproducir el c\'odigo completo (disponible en el repositorio \archivo{e1_smn}).
...
Enumera, v\'ia facetas de b\'usqueda de ESGF, todos los \archivo{source_id} de CMIP6 con \texttt{tos}/\texttt{Omon} publicado; para cada uno, agrupa sus \emph{datasets} por \archivo{grid_label}, retiene solo las variantes de grilla que cubren los tres experimentos requeridos, y elige la de mayor \archivo{nominal_resolution} (m\'as gruesa). Escribe \archivo{config/models_seed_cmip6.csv} (Secci\'on~\ref{sec:desviacion}).
...
Para cada modelo del \emph{seed}, consulta el nodo principal de ESGF (LLNL) y determina el \archivo{variant_label} (miembro de ensamble) com\'un a los tres experimentos requeridos, prefiriendo \texttt{r1i1p1f1} cuando est\'a disponible. Escribe \archivo{data/interim/models_catalog_status.csv} (estado por modelo: completo o no, fuente, miembro) y \archivo{data/interim/esgf_file_urls.json} (URLs de archivo por modelo/experimento). El script fusiona en vez de sobrescribir corridas previas: solo se protege de una nueva consulta un modelo ya marcado \texttt{complete=True}, o resuelto por una fuente alternativa (Copernicus o nodo ESGF alterno); cualquier otro se reconsulta siempre, para permitir que una corrida posterior mejore un modelo previamente incompleto (caso documentado de CESM2 en \archivo{informe/descarga_story.md}).
...
Descarga HTTP directa contra el nodo principal de ESGF, usando las URLs ya resueltas por el Paso~01.
\item \archivo{02b_search_alt_esgf_nodes.py} -- 

Para los modelos que el nodo principal no resolvi\'o, repite la b\'usqueda contra otros \'indices de la federaci\'on ESGF -- CEDA (Reino Unido), DKRZ (Alemania), IPSL (Francia), NSC (Suecia) y NCI (Australia) --, ya que un modelo puede no estar indexado en un nodo espec\'ifico por problemas puntuales de replicaci\'on y s\'i estarlo en otro.
\item \archivo{02c_download_copernicus_cds.py} -- 

\'Ultimo recurso, solo para los modelos de \archivo{config/models_copernicus_ssp245_whitelist.csv} (28 modelos verificados manualmente como disponibles en Copernicus CDS; se acota expl\'icitamente porque CDS no espeja el cat\'alogo completo de ESGF y muchos \emph{jobs} fallan con \texttt{RoocsValueError} para modelos de ``cola larga''). Requiere credenciales personales (\texttt{\textasciitilde/.cdsapirc}); si no est\'an disponibles, el paso se omite sin abortar el resto del \emph{pipeline}.
\end{enumerate}
...
Del universo de 102 modelos candidatos, 47 completaron los tres experimentos requeridos v\'ia descarga y aprobaron el control de calidad final (141 de 141 combinaciones modelo-experimento), tras el filtro de seguridad por valor descrito en la Secci\'on~\ref{sec:paso05}. Un modelo adicional, MIROC-ES2H, obtuvo \'unicamente \texttt{historical} (\texttt{ssp245} y \texttt{ssp585} no estan disponibles para descarga) y por eso no se cuenta entre los 47 completos, aunque el inventario t\'ecnico lo liste junto a ellos (Cuadro~\ref{tab:modelos}) -- ver la Figura~\ref{fig:qc}, donde aparece con dos fallos de descarga. Del resto, 44 modelos no se encontraron en ninguna fuente consultada (ESGF principal, nodos alternativos ni Copernicus CDS) y quedan descartados estructuralmente (\archivo{config/models_no_encontrado_44.csv}); un subconjunto adicional qued\'o con descarga parcial v\'ia Copernicus, sin completar los tres experimentos.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figuras/sst_QD_vf.png}
\caption[Disponibilidad y control de calidad de \texttt{tos}/\texttt{Omon} para los 102 modelos candidatos.]{Matriz de estado, por modelo y por columna (\texttt{tos}, \texttt{historical}, \texttt{ssp245}, \texttt{ssp585}), para el universo completo de 102 modelos candidatos. C\'irculo verde: dato disponible y aprobado (\texttt{tos}/\texttt{Omon} publicado, descargado y, para cada experimento, pasa el control de calidad del Paso~06 --rango f\'isico de TSM y cobertura temporal--); tri\'angulo rojo: publicado en ESGF pero nunca se logr\'o obtener el archivo (fallo de descarga, no de disponibilidad); cruz roja: descargado pero no aprueba el control de calidad; gris: el experimento no est\'a publicado para ese modelo en ning\'un nodo consultado. De los 102 candidatos, 47 completan las cuatro columnas; un modelo adicional, MIROC-ES2H, aparece con dos tri\'angulos rojos en \texttt{ssp245}/\texttt{ssp585} pero igualmente figura en el inventario t\'ecnico de 48 (Secci\'on~\ref{sec:resultados}); el resto queda excluido por al menos una columna en rojo o gris, seg\'un se detalla en \archivo{informe/descarga_story.md}.}
\label{fig:qc}
\end{figure}
...
\caption{Observado (ERSSTv5)}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.75\textwidth}
\includegraphics[width=\textwidth]{figuras/sst_2d_ACCESS-CM2.png}
\caption{Modelo: ACCESS-CM2}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.75\textwidth}
\includegraphics[width=\textwidth]{figuras/sst_2d_GFDL-CM4.png}
\caption{Modelo: GFDL-CM4}
\end{subfigure}
\caption[Comparaci\'on espacial de SST: ERSSTv5 vs.\ dos modelos CMIP6.]{Comparaci\'on espacial de la temperatura superficial del mar media del periodo hist\'orico com\'un, ya en la grilla, calendario, unidades y m\'ascara oc\'eano-tierra homogeneizados por el \emph{pipeline} (Pasos~04--05). Las tres fuentes comparten exactamente la misma grilla y la misma huella de puntos v\'alidos/faltantes, condici\'on necesaria para la comparaci\'on punto a punto formal del Entregable~2; esta figura es una verificaci\'on visual de que dicha condici\'on se cumple, no un resultado de desempe\~no.}
\label{fig:mapas2d}
\end{figure}
...
\caption[Vista previa exploratoria: TSM en Ni\~no~3.4, ERSSTv5 vs.\ 48 modelos.]{Dispersi\'on (desviaci\'on est\'andar) y mediana de la temperatura superficial del mar en la caja Ni\~no~3.4, ERSSTv5 frente a los 48 modelos del inventario, sobre el mismo periodo hist\'orico com\'un. Se incluye \'unicamente como vista previa exploratoria de que el dato qued\'o correctamente homogeneizado y es ya comparable entre fuentes; la evaluaci\'on formal de desempe\~no hist\'orico (climatolog\'ia, sesgos, diagrama de Taylor, \'indices C/E/ONI/RONI/ICEN) corresponde al Entregable~2 y no es objeto de este informe.}
\label{fig:boxplot}
\end{figure}

% ============================================================
\section{Entregable 2: proximos pasos}
\label{sec:roadmap}
% ============================================================
...
\toprule
\textbf{Entregable} & \textbf{Gr\'aficas / comparaciones exigidas} \\
\midrule
...
El Entregable~1 deja construido, verificado y versionado el insumo completo del que depender\'an los Entregables~2 a~4, de su ejecuci\'on se desprenden las siguientes conclusiones:

\begin{enumerate}[label=(\roman*)]

\item El \emph{pipeline} qued\'o completo y verificable: de 102 modelos CMIP6 candidatos, 47 completaron los tres experimentos requeridos y aprobaron el control de calidad final (141/141 combinaciones modelo-experimento) -- un modelo adicional, MIROC-ES2H, qued\'o con descarga del historico (Secci\'on~\ref{sec:resultados}) --, incluyendo la resoluci\'on de un caso real de contaminaci\'on num\'erica (FGOALS-f3-L, Secci\'on~\ref{sec:paso05}) que un control basado \'unicamente en metadatos no habr\'ia detectado.
...
\begin{itemize}
\item Al iniciar el Entregable~2, evaluar primero la representatividad f\'isica y estad\'istica del ENOS en el conjunto de 47 modelos completos (\'Indices~C y~E, correlaci\'on, sesgo, diagrama de Taylor sobre Ni\~no~3.4/1+2 espec\'ificamente) antes de dar por buena su idoneidad solo por haber pasado el control de calidad de este entregable, que verifica homogeneizaci\'on y no desempe\~no clim\'atico (Secci\'on~\ref{sec:alcance}).
