...
El resultado operativo de este entregable es un \emph{pipeline} reproducible (repositorio \archivo{e1_smn}, orquestado por un \'unico punto de entrada \texttt{run.sh}) que descarga, homogeneiza, enmascara y valida la variable \texttt{tos} (temperatura superficial del mar, tabla CMOR \texttt{Omon}) de todo modelo CMIP6 disponible con los experimentos \texttt{historical}, \texttt{ssp245}, \texttt{ssp370} y \texttt{ssp585}, junto con la referencia observacional ERSSTv5 de NOAA. Del universo de 102 modelos candidatos (46 de un barrido inicial del vocabulario CMIP6 completo m\'as 56 citados en la literatura del proyecto), 40 modelos quedaron completos en los cuatro experimentos y aprobaron el control de calidad final (160 combinaciones modelo-experimento); un modelo adicional, MIROC-ES2H, qued\'o con descarga parcial (\'unicamente \texttt{historical}, agotado tras el tope de 3 reintentos automatizados en ESGF y Copernicus CDS) y se excluye de este conteo aunque figure en el inventario t\'ecnico (Secci\'on~\ref{sec:resultados}). Entre los modelos completos se resolvi\'o, entre otros, un bug de contaminaci\'on num\'erica de FGOALS-f3-L mediante un filtro de seguridad por valor f\'isico (Secci\'on~\ref{sec:paso05}), y dos bugs de robustez de la propia descarga/verificaci\'on de ESGF: paginaci\'on de b\'usqueda de archivos que pod\'ia cortarse a mitad ante un corte de red transitorio (Secci\'on~\ref{sec:paso01}) y chunks crudos de m\'as de una variante de grilla mezclados en un mismo modelo (Secci\'on~\ref{sec:paso04}).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{cmip6_contribution.png}
...
\begin{longtable}{@{}L{1.1cm}L{2.4cm}L{2.4cm}L{2.1cm}L{2.1cm}L{1.9cm}@{}}
...
MIROC-ES2H obtuvo \'unicamente \texttt{historical} (Secci\'on~\ref{sec:resultados}) y por eso no figura en este cuadro, aunque el \emph{pipeline} lo siga rastreando (agot\'o el tope de 3 reintentos autom\'aticos en ESGF y Copernicus CDS, Secci\'on~\ref{sec:paso02}).
...
Un modelo que agota (b) y (c) sin resolverse suma 1 a la columna \archivo{intentos_busqueda} del cat\'alogo (Secci\'on~\ref{sec:paso01}); al llegar a 3 intentos en corridas separadas, deja de reintentarse autom\'aticamente (caso de \texttt{MIROC-ES2H}: ESGF completo + Copernicus, sin \'exito) hasta que se borre el cat\'alogo a mano.
...
Del universo de 102 modelos candidatos, 40 completaron los cuatro experimentos requeridos v\'ia descarga y aprobaron el control de calidad final (160 combinaciones modelo-experimento), tras el filtro de seguridad por valor descrito en la Secci\'on~\ref{sec:paso05} y las correcciones de robustez de descarga/verificaci\'on de ESGF descritas en las Secciones~\ref{sec:paso01} y~\ref{sec:paso04}. Un modelo adicional, MIROC-ES2H, obtuvo \'unicamente \texttt{historical} (\texttt{ssp245}, \texttt{ssp370} y \texttt{ssp585} no est\'an disponibles para descarga, agotado tras el tope de 3 reintentos autom\'aticos en ESGF y Copernicus CDS, Secci\'on~\ref{sec:paso02}) y por eso no se cuenta entre los 40 completos, aunque el \emph{pipeline} lo siga rastreando (ver la Figura~\ref{fig:qc}). Del resto, 44 modelos no se encontraron en ninguna fuente consultada (ESGF principal, nodos alternativos ni Copernicus CDS) y quedan descartados estructuralmente; un subconjunto adicional qued\'o con descarga parcial v\'ia Copernicus, sin completar los cuatro experimentos.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figuras/sst_QD_vf.png}
\caption[Disponibilidad y control de calidad de \texttt{tos}/\texttt{Omon} para los 102 modelos candidatos.]{Matriz de estado, por modelo (identificados por su c\'odigo \texttt{M001}--\texttt{M102} y su nombre, Secci\'on~\ref{sec:tabla_modelos}) y por columna (\texttt{tos}, \texttt{historical}, \texttt{ssp245}, \texttt{ssp370}, \texttt{ssp585}), para el universo completo de 102 modelos candidatos. C\'irculo verde relleno (PASS): descargado y aprueba el control de calidad del Paso~06; cruz roja (FAIL): descargado pero no aprueba el control de calidad; c\'irculo verde hueco (PENDIENTE): existe un enlace de descarga vivo pero todav\'ia no se descarg\'o o proces\'o; c\'irculo gris hueco (SIN\_LINK): el modelo existe seg\'un el Paso~00b pero el Paso~01 no encontr\'o ning\'un enlace de descarga vivo; c\'irculo naranja hueco (SOLO\_HIST1950, solo en la columna \texttt{historical}): el modelo publica \texttt{hist-1950} (HighResMIP, 1950--2014) en vez de \texttt{historical} est\'andar (1850--2014) -- equivalente para fines de clasificaci\'on en este proyecto, pero nunca elegible como \emph{seed} porque los modelos HighResMIP que solo publican \texttt{hist-1950} corren \texttt{highres-future} en vez de los SSP est\'andar, y por lo tanto nunca tienen los cuatro experimentos requeridos; cuadrado rojo (UNAVAILABLE): el experimento no existe en absoluto para ese modelo en ning\'un nodo consultado. De los 102 candidatos, 40 completan las cinco columnas; MIROC-ES2H aparece con FAIL/UNAVAILABLE en \texttt{ssp245}/\texttt{ssp370}/\texttt{ssp585} pero el \emph{pipeline} lo sigue rastreando (Secci\'on~\ref{sec:resultados}); el resto queda excluido por al menos una columna sin PASS.}
\label{fig:qc}
\end{figure}
...
\caption{Modelo: ACCESS-CM2 (M001)}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.75\textwidth}
\includegraphics[width=\textwidth]{figuras/sst_2d_ACCESS-ESM1-5.png}
\caption{Modelo: ACCESS-ESM1-5 (M002)}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.75\textwidth}
\includegraphics[width=\textwidth]{figuras/sst_2d_GISS-E2-1-G.png}
\caption{Modelo: GISS-E2-1-G (M057)}
\end{subfigure}
\caption[Comparaci\'on espacial de SST: ERSSTv5 vs.\ los tres modelos de grilla m\'as gruesa.]{Comparaci\'on espacial de la temperatura superficial del mar media del periodo hist\'orico com\'un (mes de muestra 1950-12), ya en la grilla, calendario, unidades y m\'ascara oc\'eano-tierra homogeneizados por el \emph{pipeline} (Pasos~04--05); el c\'odigo en la esquina superior izquierda de cada panel de modelo es su identificador \texttt{M001}--\texttt{M102} (Cuadro~\ref{tab:modelos}). Las cuatro fuentes comparten exactamente la misma grilla y la misma huella de puntos v\'alidos/faltantes, condici\'on necesaria para la comparaci\'on punto a punto formal del Entregable~2; esta figura es una verificaci\'on visual de que dicha condici\'on se cumple incluso en el caso m\'as exigente (grilla nativa m\'as gruesa), no un resultado de desempe\~no.}
\label{fig:mapas2d}
\end{figure}
...
\includegraphics[width=\textwidth]{figuras/serie_ACCESS-CM2_sst_Nino3.4.png}
\caption{ACCESS-CM2 -- Ni\~no~3.4}
\end{subfigure}
\begin{subfigure}[t]{\textwidth}
\includegraphics[width=\textwidth]{figuras/serie_ACCESS-ESM1-5_sst_Nino3.4.png}
\caption{ACCESS-ESM1-5 -- Ni\~no~3.4}
\end{subfigure}
\begin{subfigure}[t]{\textwidth}
\includegraphics[width=\textwidth]{figuras/serie_GISS-E2-1-G_sst_Nino3.4.png}
\caption{GISS-E2-1-G -- Ni\~no~3.4}
\end{subfigure}
\caption[Series de tiempo de TSM en Ni\~no~3.4 para los tres modelos de grilla m\'as gruesa.]{Serie de tiempo de la TSM promediada en la caja Ni\~no~3.4 (1850--2100), \texttt{historical} (gris) y los tres escenarios SSP superpuestos (Secci\'on~\ref{sec:dominios_periodos}), para los mismos tres modelos de la Figura~\ref{fig:mapas2d}. L\'inea fina con marcador de punto en cada dato mensual real, sin interpolar huecos: un tramo sin puntos es directamente visible como un hueco real de datos (chunk nunca descargado), en vez de quedar disimulado por una l\'inea continua entre dos meses lejanos. La caja Ni\~no~1+2 (no incluida aqu\'i por espacio) se genera de forma an\'aloga en \archivo{figures/serie\_<modelo>\_sst\_Nino1+2.png}.}
\label{fig:series}
\end{figure}
...
\item El \emph{pipeline} qued\'o completo y verificable: de 102 modelos CMIP6 candidatos, 40 completaron los cuatro experimentos requeridos y aprobaron el control de calidad final (160 combinaciones modelo-experimento) -- un modelo adicional, MIROC-ES2H, qued\'o con descarga parcial del historico, agotado tras el tope de reintentos (Secci\'on~\ref{sec:resultados}) --, incluyendo la resoluci\'on de un caso real de contaminaci\'on num\'erica (FGOALS-f3-L, Secci\'on~\ref{sec:paso05}) que un control basado \'unicamente en metadatos no habr\'ia detectado.
