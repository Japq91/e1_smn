...
De ahí en más, alcanza con `./run.sh` — **no hace falta `conda activate` a mano**: `run.sh` activa el entorno `smn_toe` solo, si lo encuentra creado (ver Convenciones más abajo si igual te aparece un error de dependencias faltando).
