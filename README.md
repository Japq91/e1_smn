...
Este paso define qué puntos de la grilla se consideran válidos para el análisis: se construye una máscara 1=océano a partir de los puntos válidos de ERSSTv5 (un punto se considera océano si registra al menos un mes válido en todo su periodo observado), y se aplica por igual a los nueve archivos modelo×experimento generados en el paso 04. Al derivarse del observado en vez de usar `sftlf` por modelo, se garantiza que todas las fuentes —modelos y ERSSTv5— compartan exactamente la misma huella válida/faltante, condición necesaria para que las comparaciones posteriores (control de calidad, boxplots, cálculo de TOE) sean consistentes entre sí. Salida: `data/processed/masked/`, el dato final (1841/2016 puntos de océano, 91.3 % del dominio).

### 06 — Control de calidad (`06_qc_checks.py`)
Este paso actúa como filtro de aceptación del pipeline: por archivo, evalúa el rango físico de la SST (−2 a 39 °C) y la longitud temporal esperada (10 % de tolerancia), y clasifica cada combinación modelo-experimento como PASS o FAIL en `qc_report.csv`. Es la última verificación antes de que un dato se considere confiable para el cálculo de TOE, y el criterio que usa el paso 07 para decidir qué modelos integran el inventario final. Actualmente, las nueve combinaciones modelo×experimento disponibles superan el control.
